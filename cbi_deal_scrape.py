...
def save_data_to_csv(save_path: str, row: list[str], file_exists: Optional[bool] = False):
...
def get_n_pages(driver: webdriver.ChromeDriver) -> float:
...
def log_in(driver: webdriver.ChromeDriver, username: str, password: str) -> None:
...
def next_page(driver: webdriver.ChromeDriver) -> None:
...
def scrape_row(driver: webdriver.ChromeDriver) -> list[Any]:
...
def scrape_year(link: str, save_path: str, username: str, password: str) -> None:
    ...
    for page in range(1, n_pages + 1):
        ...
        driver.implicitly_wait(10) # allow the webpage to load before searching for elements
...
def scrape_15_21() -> None:
    ...
    for year in range(15, 22):
        ...
        scrape_year(data_link, csv_save_path, USERNAME, PASSWORD)
