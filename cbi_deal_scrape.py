...
HEADER = ['Companies', 'Investment Stage', 'Deal Size', 'Deal Date', 'Round Investors', 'Description', 'Industry', 'Country', 'Total Funding', 'All Investors']
...
class DataScraper:
    ...
    def scrape_year(self, link: str, save_path: str) -> None:
        ...
        self.save_data_to_csv(save_path, HEADER, file_exists=False)

        for page in range(1, n_pages + 1):
            print('Scraping page %s out of %s pages' % (page, n_pages))

            self.driver.implicitly_wait(10) # allow the webpage to load before searching for elements
            ...
            for _ in range(1, 26):
                ...
                """
                try:
                    company_ls = self.scrape_row(self.driver)
            ...
            self.save_data_to_csv(save_path, companies_ls, file_exists=True)
            self.next_page()    

    def scrape_row(self) -> list[Any]:
        ...
        for col in range(2, 12):
            # Scraping ['Companies']
            if col in [2]:
                c_col = self.driver.find_element(By.XPATH, '//*[@id="react-tabs-5"]/div/div/div[1]/div/div[2]/div[%s]/div/div[%s]/div/div/div/div[1]/div/div/div/div' % (row, col))
                company_ls.append(c_col.text)   
            # Scraping ['Investment Stage', 'Deal Size', 'Country', 'Total Funding']
            elif col in [3, 4, 9, 10]:
                c_col = self.driver.find_element(By.XPATH, '//*[@id="react-tabs-5"]/div/div/div[1]/div/div[2]/div[%s]/div/div[%s]/div/div/div[1]/div/div/div' % (row, col))
                company_ls.append(c_col.text)
            # Scraping ['Round Investors', 'All Investors']
                ...
                while more_rows:
                    try:
                        ...
                        c_col = self.driver.find_element(By.XPATH, '//*[@id="react-tabs-5"]/div/div/div[1]/div/div[2]/div[%s]/div/div[%s]/div/div/div[1]/div/div/div/div/div[%s]/div/div/div/a' % (row, col, sub_row))
            ...
            # Scraping  ['Deal Date']
                ...
                c_col = self.driver.find_element(By.XPATH, '//*[@id="react-tabs-5"]/div/div/div[1]/div/div[2]/div[%s]/div/div[%s]/div/div/div[1]/div/div/p' % (row, col))
                company_ls.append(c_col.text)
            # Scraping ['Description', 'Industry']
            elif col in [7, 8]:
                c_col = self.driver.find_element(By.XPATH, '//*[@id="react-tabs-5"]/div/div/div[1]/div/div[2]/div[%s]/div/div[%s]/div/div/div[1]/div/div/div/div' % (row, col))
                company_ls.append(c_col.text)
    ...
    def log_in(self) -> None:
        ...
        self.driver.find_element(By.XPATH, '//*[@id="hero"]/div/section/form/div[3]/button').click()
    ...
    def next_page(self) -> None:
        ...
        next_page_button = self.driver.find_element(By.XPATH, '//*[@id="react-tabs-5"]/div/div/div[2]/div/div/div[3]/div/div/button[12]')
    ...
    def get_n_pages(self) -> float:
        ...
        n_deals_html = self.driver.find_element(By.XPATH, '//*[@id="react-tabs-5"]/div/div/div[2]/div/div/div[1]/div')
    ...
    def save_data_to_csv(self, save_path: str, row: list[str], file_exists: Optional[bool] = False):
